...
BASE_INSULT = "tontolaba"

def worker(server_uri, requests):
    try:
        server = xmlrpc.client.ServerProxy(server_uri)
        for i in range(requests):
            server.add_insult(f"{BASE_INSULT}_{i}")
    except Exception as e:
        print(f"Error en worker (InsultClient): {e}")
...
with open("tiempos_clientes.log", "a") as f:
    f.write(
        f"InsultClient,{NUM_REQUESTS},{len(servers)},"
        f"{total_time:.4f}\n"
    )

print("InsultClient Acabado")
