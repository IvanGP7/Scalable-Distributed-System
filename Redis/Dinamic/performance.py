...
df = pd.read_csv("Redis/Dinamic/tiempos_clientes.log", header=None, names=["Client", "Nodos", "Iteraciones", "Tiempo"])
...
for client in df["Client"].unique():
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"Redis/Dinamic/tiempo_vs_nodos_{client}.png")  # Guarda cada gráfico en un archivo
...
for client in df["Client"].unique():
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"Redis/Dinamic/speedup_vs_nodos_{client}.png")  # Guarda cada gráfico en un archivo
