...
df = pd.read_csv("Pyro/tiempos_clientes.log", header=None, names=["Client", "Iteraciones", "Nodos", "Tiempo"])
...
for client in df["Client"].unique():
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"Pyro/tiempo_vs_nodos_{client}.png")  # Guarda cada gráfico en un archivo
...
for client in df["Client"].unique():
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"Pyro/speedup_vs_nodos_{client}.png")  # Guarda cada gráfico en un archivo
